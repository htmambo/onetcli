...
- **标题**：审计 i18n / 文案类计划的完成度时，用脚本扫"产品代码中的中文字符串字面量"，不要凭 locales 文件存在下结论。
- **触发信号**：需要回答"某国际化 / 文案迁移计划还剩多少没做"；或 yml 文件都在但怀疑代码里仍有硬编码。
- **根因 / 约束**：locales/*.yml 文件存在只说明基础设施就位（i18n! 宏 + 依赖），不代表字符串已迁移；裸 `rg '\p{Han}'` 会把中文注释、doc 注释里引号包裹的中文示例、以及 `#[cfg(test)]` 模块内的测试断言全部计入，假阳性极高（本仓实测 2000+ 处里产品代码只占约一半）。
- **正确做法**：用脚本逐文件处理（参考曾用的 `/tmp/scan_han.py` 模式）：匹配 `"[^"]*\p{Han}[^"]*"` 字面量、剔除 `//` 注释行、在首个 `#[cfg(test)]` 处截断文件（注意 `mod tests;` 单独文件也要确认其 cfg 门控）；按 crate 汇总后再抽样确认字面量确为用户可见。
- **验证方式**：抽样头部文件的命中行肉眼核对；产物计数与后续迁移工作量估算吻合。
- **适用范围**：所有"按 crate 迁移硬编码字符串"类计划的进度审计与剩余量估算。
