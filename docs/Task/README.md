...
### 2026-09
- ✅ [LLM 提供商云同步接入](Archive/2026-09/LLM_PROVIDER_SYNC_PLAN.md) - 归档于 2026-09-20（审计确认 §10 全部 10 步已落地：llm_providers 三同步列 + `SyncableItem for ProviderConfig` + repository 云同步方法（含 pending_cloud_deletions 墓碑）+ `data_type::LLM_PROVIDER` + service upload/decrypt + `LlmProviderSyncType` handler 注册 engine.rs:98 + 可选 Web 端 syncItemType.ts/syncPayloadTable.ts 均已支持）
- ✅ [修复 Round 01：安全 & 性能 16 项](Archive/2026-09/FIX_ROUND_01_PLAN.md) - 完成于 2026-09-20（A1 SSH 私钥字段级加密（方案 A 推迟自动迁移）+ A2/A3 文件权限 0600 + B1 MySQL NO_BACKSLASH_ESCAPES + B2 过滤器列名加引号（IdentifierQuote）+ B3 SSH 首次连接 TOFU 确认（哨兵 + 暂存表 + 覆盖层按钮）+ B4 IPC 密码 env 通路收敛（双重闸门，随后随子系统整体移除）+ B5 SFTP 路径越界校验 + B6 TLS 风险提示 + P1-P6 性能项；分支 `fix/round-01-security-and-perf`，commit `73b03a6e` → `c8eb6a8c`；遗留：A1 迁移函数接入窗口期约 2026-11 中旬）
- ✅ [彻底移除外部数据库驱动子系统](Archive/2026-09/REMOVE_EXTERNAL_DB_DRIVERS_PLAN.md) - 完成于 2026-09-20（方案 A：保留 `DatabaseType::External` 变体兜底存量数据，能力全移除；删 `crates/db/src/ipc/` 4219 行 + 3 个依赖；编辑/复制拦截 + 列表"已停止支持"提示；排除 database_view_plugin 两个 `.expect` panic 炸弹；顺带修复 7 个既有红灯测试；分支 `refactor/remove-external-db-drivers`，commit `f21b6cfd` → `1a157c09`；用户实测 MySQL/MongoDB 正常）
...
- ✅ [rust_i18n 全量补全](Archive/2026-09/RUST_I18N_COMPLETION_PLAN.md) - 完成于 2026-09-20（Task 1-16 全部完成；新增约 300+ i18n key，覆盖 11 个 crate 的 locales yml；迁移口径：UI 文案/错误消息/warn-error 日志；跳过 expect/panic 文案、测试代码、debug/info 日志、LLM prompt/协议字段；终扫残留 261 处均为有意跳过并分类说明；`cargo check --workspace` 通过；main 105/105 + terminal_view 221/221 + one-core 236/236 + terminal 97/97 + db 404/404 + db_view 313/313 + gpui-component 36/36 均通过；经验沉淀到 AGENTS.md"已验证经验"首条）
