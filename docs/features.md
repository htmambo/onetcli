---
title: 功能
description: 了解 OmniHub 如何用纯 Rust、GPUI 和 GPU 渲染整合数据库、Redis、MongoDB、SSH/SFTP、终端与 AI 工作流
---

# 功能

OmniHub 是一个纯 Rust 构建的高性能一体化运维工作台。它不是 WebView 套壳，而是基于 GPUI 构建的原生桌面应用，用一套工作区收拢数据库、Redis、MongoDB、SSH/SFTP、终端和 AI 辅助能力。

![OmniHub 主应用工作台](/screenshots/app.png)

## 原生桌面性能

OmniHub 走 GPUI 原生界面路线，面向长时间打开、高频切换连接和处理大量工程上下文的使用方式设计。
...
OmniHub 将多种数据库连接放进同一个桌面工作台，适合日常查询、结构浏览、数据检查和问题排查。
...
Redis 不是附属入口，而是独立工作流。OmniHub 提供 Redis Key 浏览、值查看、缓存排查和集群场景支持，适合开发调试、线上缓存确认和运维排查。
...
OmniHub 支持 SSH 远程终端、SFTP 文件管理和本地终端。远程连接、文件传输、脚本执行和日志排查可以沿用同一套上下文。
...
OmniHub 支持从应用内打开远程文件，并提供语法高亮和自动补全。处理配置文件、脚本和日志片段时，不需要在终端、编辑器和文件传输工具之间来回切换。

![远程文件编辑器](/screenshots/remote_file_editor.png)

## ER 图与服务器监控

OmniHub 也覆盖数据库结构理解和基础服务器监控场景：
...
AI 在 OmniHub 中是贴近任务上下文的辅助层，而不是单独占据主叙事的聊天入口。
