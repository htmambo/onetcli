...
const ENABLE_ENV: &str = "OMNIHUB_DOCKER_E2E";
...
async fn docker_local_and_dynamic_forwarding_roundtrip() -> Result<()> {
    ...
    let ssh_port = env_u16("OMNIHUB_DOCKER_SSH_PORT", 2222)?;
    let ssh_user = env_string("OMNIHUB_DOCKER_SSH_USER", "omnihub");
    let ssh_password = env_string("OMNIHUB_DOCKER_SSH_PASSWORD", "omnihub-pass");
    let target_host = env_string("OMNIHUB_DOCKER_TARGET_HOST", "omnihub-pf-target");
    let target_port = env_u16("OMNIHUB_DOCKER_TARGET_PORT", 80)?;
