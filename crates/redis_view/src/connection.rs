...
fn resolve_private_key_content(key_value: &str) -> Result<String, RedisError> {
    ...
    if FsPath::new(trimmed).is_file() {
        return std::fs::read_to_string(trimmed).map_err(|error| {
            RedisError::connection(format!("failed to read ssh private key file: {error}"))
        });
    }
...
impl RedisConnectionImpl {
    ...
    fn connection_config_for_endpoint(
        ...
        let mut db_config = Self::connection_config_for_db(config, db)?;
        db_config.host = host.to_string();
        db_config.port = port;
        // 隧道已在本地建立，后续 Redis 客户端连本地转发地址，不再二次建隧道。
        db_config.ssh_tunnel = None;
        Ok(db_config)
    }
    ...
    async fn resolve_connection_target(
        ...
        let Some(tunnel_config) = config.ssh_tunnel.as_ref() else {
        ...
        let timeout_secs = tunnel_config
            .timeout
            .filter(|value| *value > 0)
            .unwrap_or(DEFAULT_SSH_TIMEOUT_SECONDS);

        let ssh_config = SshConnectConfig {
            host: ssh_host,
            port: tunnel_config.port,
            username: ssh_username,
            auth: build_ssh_auth(tunnel_config)?,
    ...
    async fn open_connection_for_db(
        ...
        Self::open_connection_for_endpoint(config, db, &config.host, config.port).await
    }
