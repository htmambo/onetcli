...
impl SqlEditorTab {
    ...
    fn execute_sql_text(&mut self, sql: String, window: &mut Window, cx: &mut Context<Self>) {
        ...
        if self.uses_schema_as_database {
            if current_schema_value.is_none() {
                window.push_notification(t!("Query.please_select_schema").to_string(), cx);
                return;
            }
        } else if current_database_value.is_none() {
    ...
    pub fn save_and_close(
        ...
        cx.emit(SqlEditorEvent::QuerySaved {
            ...
            database: if self.uses_schema_as_database {
                self.schema_select.read(cx).selected_value().cloned()
            } else {
    ...
    fn handle_explain_sql(&mut self, _: &ClickEvent, window: &mut Window, cx: &mut Context<Self>) {
        ...
        let Ok(plugin) = DbManager::default().get_plugin(&self.database_type) else {
            window.push_notification(t!("SqlEditorView.db_plugin_not_found").to_string(), cx);
            return;
        };

        let Some(explain_sql) = plugin.build_explain_sql(&sql) else {
            window.push_notification(t!("SqlEditorView.explain_only_query").to_string(), cx);
            return;
        };
            ...
            .child(
                ...
                h_flex()
                    ...
                    .when(
                        ...
                        |this| {
                            this.child(
                                // Schema selector（含 schema-as-database 模式）
