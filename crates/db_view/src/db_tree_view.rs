...
fn connection_node(id: String, name: String, config: &DbConnectionConfig) -> DbNode {
    ...
    apply_connection_node_config(&mut node, config);
    node
}

fn connection_node_icon(node: &DbNode) -> Icon {
    // 外部驱动已移除，节点图标统一按数据库类型兜底
    node.database_type.as_node_icon()
}

fn apply_connection_node_config(node: &mut DbNode, config: &DbConnectionConfig) {
    ...
    fn update_connection_info(&mut self, connection: &StoredConnection, cx: &mut Context<Self>) {
        if let Ok(config) = connection.to_db_connection() {
            ...
            if let Some(node) = self.db_nodes.get_mut(&id) {
                apply_connection_node_config(node, &config);
                let mut global_db_state = cx.global_mut::<GlobalDbState>().clone();
