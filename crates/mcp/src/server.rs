...
pub trait McpHandler: Send + Sync {
    ...
    async fn read_resource(&self, params: ReadResourceParams) -> Result<ReadResourceResult, McpError> {
        Err(McpError::method_not_found(&format!("resource:{}", params.uri)))
    }
    ...
    async fn get_prompt(&self, params: GetPromptParams) -> Result<GetPromptResult, McpError> {
        Err(McpError::method_not_found(&format!("prompt:{}", params.name)))
    }
...
impl<T: Transport> McpServer<T> {
    pub fn new(transport: T, handler: Arc<dyn McpHandler>) -> Self {
        Self { transport: Arc::new(transport), handler, protocol_version: RwLock::new(String::new()) }
    }
        ...
        loop {
            ...
            let msg: JsonRpcMessage = match serde_json::from_slice(&raw) {
                ...
                Err(e) => {
                    ...
                    let resp =
                        JsonRpcResponse::error(RequestId::Number(0), JsonRpcError::new(-32700, e.to_string()));
    ...
    async fn dispatch_request(&self, req: JsonRpcRequest) -> JsonRpcResponse {
        ...
        let result = match req.method.as_str() {
            "initialize" => {
                ...
                match params {
                    Ok(p) => {
                        match self.handler.initialize(p).await {
                            Ok(r) => {
                                *self.protocol_version.write().await = r.protocol_version.clone();
                                JsonRpcResponseResult::Success(serde_json::to_value(r).unwrap())
                            }
                            Err(e) => JsonRpcResponseResult::Error(JsonRpcError::new(e.code.code(), e.message.clone())),
                        }
                    }
                    Err(e) => JsonRpcResponseResult::Error(JsonRpcError::new(-32602, e.to_string())),
                }
            }
            "tools/list" => {
            ...
            "tools/call" => {
                ...
                match params {
                    Ok(p) => {
                        match self.handler.call_tool(p).await {
                            Ok(r) => JsonRpcResponseResult::Success(serde_json::to_value(r).unwrap()),
                            Err(e) => JsonRpcResponseResult::Error(JsonRpcError::new(e.code.code(), e.message.clone())),
                        }
                    }
            ...
            "resources/templates/list" => {
                match self.handler.list_resource_templates().await {
                    Ok(r) => JsonRpcResponseResult::Success(serde_json::to_value(r).unwrap()),
                    Err(e) => JsonRpcResponseResult::Error(JsonRpcError::new(e.code.code(), e.message.clone())),
                }
            }
            "resources/read" => {
                ...
                match params {
                    Ok(p) => {
                        match self.handler.read_resource(p).await {
                            Ok(r) => JsonRpcResponseResult::Success(serde_json::to_value(r).unwrap()),
                            Err(e) => JsonRpcResponseResult::Error(JsonRpcError::new(e.code.code(), e.message.clone())),
                        }
                    }
            ...
            "prompts/get" => {
                ...
                match params {
                    Ok(p) => {
                        match self.handler.get_prompt(p).await {
                            Ok(r) => JsonRpcResponseResult::Success(serde_json::to_value(r).unwrap()),
                            Err(e) => JsonRpcResponseResult::Error(JsonRpcError::new(e.code.code(), e.message.clone())),
                        }
                    }
            ...
            other => {
                JsonRpcResponseResult::Error(JsonRpcError::new(
                    -32601,
    ...
    async fn dispatch_notification(&self, notif: JsonRpcNotification) {
        match notif.method.as_str() {
            ...
            "notifications/message" => {
                if let Ok(msg) =
                    serde_json::from_value::<LoggingMessageNotification>(notif.params.unwrap_or_default())
                {
                    self.handler.handle_logging_message(msg).await;
                }
            }
            "notifications/roots_changed" => {
                self.handler.handle_roots_changed(RootsListChangedNotification {}).await;
            }
            _ => {}
        }
    }
