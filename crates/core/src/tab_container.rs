...
fn default_inactive_tab_color(tab_bar_color: gpui::Hsla, is_dark: bool) -> gpui::Hsla {
    with_alpha(shift_tab_tone(tab_bar_color, is_dark, 0.18), 1.0)
}
...
fn resolve_inactive_tab_color(
    ...
    explicit_inactive_tab_color.unwrap_or_else(|| {
        if explicit_tab_bar_color.is_some() {
            default_inactive_tab_color(resolved_tab_bar_color, is_dark)
        } else {
            theme_tab_color
        }
    })
