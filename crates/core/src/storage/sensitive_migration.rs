...
//! 3. 通过函数自身幂等检查保证多次启动不破坏数据（每次解锁都重扫，
//!    以收敛过渡期内老客户端经云同步写回的迟到明文；见函数级文档）
//!
//! 安全约束：
...
/// 启动期执行一次敏感字段加密迁移。
///
/// # 调用时机（2026-09-20 起已接入）
///
/// 在 `main/src/home_tab.rs` 主密钥对话框 `on_close` 中、
/// `crypto::has_master_key()` 为真时调用（覆盖首次设置 / 解锁 / 改密
/// 三条路径）。原"方案 A 推迟接入"经用户确认提前启用。
///
/// # 为什么不注册 `_migrations` 版本门
///
/// 计划原方案要求注册版本号防重复执行，但该设计只在"所有客户端均已升级"
/// 的收尾期成立。提前启用后，老客户端仍可能经云同步把明文 `ssh_private_key`
/// 随时写回本地；版本门会让这些迟到明文永久漏迁。因此改为**每次解锁都跑**：
/// 函数幂等（已加密字段跳过），代价为两表全扫 + JSON 解析，连接表规模下
/// 可忽略。
///
/// # 函数行为
///
...
/// # 幂等
///
/// 函数重复调用不会破坏已加密数据；`crypto::is_encrypted` 已加密的字段
/// 不会被二次加密。
///
/// # 兼容性说明
///
/// 详见 `docs/plans/2026-09-18-fix-round-01.md` 的"兼容性策略：方案 A"章节
/// （已于 2026-09-20 经用户授权提前启用，风险见该节兼容性矩阵"新 → 老"行）。
