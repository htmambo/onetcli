...
mod provider_registry_tests {
    ...
    fn provider_registry_loads_sorted_providers() {
        ...
        write_provider(temp.path(), "vnc", "VNC", "vnc", "./omnihub-vnc-helper");
        write_provider(temp.path(), "rdp", "RDP", "rdp", "./omnihub-rdp-helper");
    ...
    fn provider_entry_command_remains_relative_when_file_exists_in_manifest_dir() {
        ...
        write_provider(temp.path(), "rdp", "RDP", "rdp", "./omnihub-rdp-helper");
        fs::write(
            temp.path().join("rdp").join("omnihub-rdp-helper"),
            b"helper",
        )
        .unwrap();
        ...
        assert_eq!("./omnihub-rdp-helper", provider.entry.command);
    }
