...
pub struct X11WindowState {
    ...
    deepin_no_titlebar_supported: bool,
...
impl X11WindowState {
    pub fn new(
        ...
        let setup_result = maybe!({
            ...
            const WM_HINT_INPUT: u32 = 1 << 0;
            const WM_HINT_STATE: u32 = 1 << 1;
            const WM_HINT_WINDOW_GROUP: u32 = 1 << 6;
            const WM_STATE_NORMAL: u32 = 1;

            let wm_hints = [
                WM_HINT_INPUT | WM_HINT_STATE | WM_HINT_WINDOW_GROUP,
                1,
                WM_STATE_NORMAL,
                0,
                0,
            ...
            check_reply(
                || "X11 ChangeProperty for WM_HINTS failed.",
                xcb.change_property(
                    xproto::PropMode::REPLACE,
                    x_window,
                    ...
                    32,
                    wm_hints.len() as u32,
                    bytemuck::cast_slice::<u32, u8>(&wm_hints),
                ),
            )?;

            check_reply(
                || "X11 ChangeProperty for WM_CLIENT_LEADER failed.",
                xcb.change_property32(
                    xproto::PropMode::REPLACE,
                    x_window,
            ...
            if let Some(size) = params.window_min_size {
                ...
                size_hints.win_gravity = Some(xproto::Gravity::NORTH_WEST);
                check_reply(
                    || {
                        format!(
                            "X11 change of WM_SIZE_HINTS failed. position: ({}, {}), size: ({}, {}), min_size: {:?}",
                            bounds.origin.x.0,
                            bounds.origin.y.0,
                            bounds.size.width.0,
                            bounds.size.height.0,
                ...
                check_reply(
                    || {
                        format!(
                            "X11 change of WM_SIZE_HINTS failed. position: ({}, {}), size: ({}, {})",
                            bounds.origin.x.0,
                            bounds.origin.y.0,
                            bounds.size.width.0,
                            bounds.size.height.0
...
fn maximized_wm_hint_property_state(is_maximized: bool) -> WmHintPropertyState {
    if is_maximized {
...
impl PlatformWindow for X11Window {
    ...
    fn zoom(&self) {
        ...
        self.set_wm_hints(
            ...
            maximized_wm_hint_property_state(self.is_maximized()),
    ...
    fn request_decorations(&self, mut decorations: crate::WindowDecorations) {
        ...
        {
            log::info!(
                "x11: client-side decorations unsupported, falling back to server-side window decorations"
            );
        ...
        let hints_data: [u32; 5] = match decorations {
            // Deepin 25 在 X11 下对系统装饰窗口更依赖完整的 Motif functions/decorations 声明。
            // 仅声明 decorations 字段会让标题栏按钮显示出来，但最大化后的“还原”主动作无法生效。
            ...
            WindowDecorations::Client => [MWM_HINTS_DECORATIONS, 0, 0, 0, 0],
        };
        ...
        if state.deepin_no_titlebar_supported {
            let deepin_no_titlebar = match decorations {
                WindowDecorations::Server => 0,
                WindowDecorations::Client => 1,
            };
            let success = check_reply(
                || "X11 ChangeProperty for _DEEPIN_NO_TITLEBAR failed.",
                self.0.xcb.change_property32(
                    xproto::PropMode::REPLACE,
                    self.0.x_window,
                    state.atoms._DEEPIN_NO_TITLEBAR,
                    xproto::AtomEnum::CARDINAL,
                    &[deepin_no_titlebar],
                ),
            )
            ...
            let deepin_force_decorate = match decorations {
                WindowDecorations::Server => 1,
                WindowDecorations::Client => 0,
            };
            check_reply(
                || "X11 ChangeProperty for _DEEPIN_FORCE_DECORATE failed.",
                self.0.xcb.change_property32(
                    xproto::PropMode::REPLACE,
                    self.0.x_window,
                    state.atoms._DEEPIN_FORCE_DECORATE,
                    xproto::AtomEnum::CARDINAL,
                    &[deepin_force_decorate],
                ),
            )
            .log_err();
        }
...
mod tests {
    ...
    fn maximized_windows_use_remove_for_restore() {
        assert_eq!(
            maximized_wm_hint_property_state(true) as u32,
            WmHintPropertyState::Remove as u32
        );
    }

    #[test]
    fn windowed_windows_use_add_for_maximize() {
        assert_eq!(
