...
impl X11Client {
    pub(crate) fn new() -> anyhow::Result<Self> {
        ...
        let gtk_frame_extents_supported =
            check_root_atom_supported(&xcb_connection, &atoms, root, atoms._GTK_FRAME_EXTENTS);
        let deepin_no_titlebar_supported =
            check_root_atom_supported(&xcb_connection, &atoms, root, atoms._DEEPIN_NO_TITLEBAR);
        let client_side_decorations_supported =
            deepin_no_titlebar_supported || (compositor_present && gtk_frame_extents_supported);
        log::info!(
            "x11: compositor present: {}, gtk_frame_extents_supported: {}, deepin_no_titlebar_supported: {}",
            compositor_present,
