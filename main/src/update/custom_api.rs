    ...
    fn select_sha256_prefers_target_triple_before_global_fallback() {
        let response = serde_json::from_str::<UpdateResponse>(
            r#"{
                ...
                "sha256": "global-hash",
                "sha256s": {
                    "x86_64-unknown-linux-gnu": "linux-hash"
                }
            }"#,
        )
        .unwrap();
        ...
        assert_eq!(
            Some("linux-hash".to_string()),
            select_sha256_for_keys(&response, &["x86_64-unknown-linux-gnu", "linux"])
        );
    }

    #[test]
    fn select_fallback_download_url_supports_github_aliases() {
        let response = serde_json::from_str::<UpdateResponse>(
            r#"{
                "version": "1.2.3",
                "github_download_url": "https://github.test/default.bin",
                "github_downloads": {
                    "windows": "https://github.test/windows.zip"
                }
            }"#,
        )
        .unwrap();
